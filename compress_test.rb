...
def do_test
  ...
  test_data = {
    "original" => [3,7,5,2,8,5,4,1], 
    "8_top" => [4.375,-0.125,0.75,2.0,-2.0,1.5,1.5,1.5],
    "4_top" => [4.375,-0.125,0.75,2.0, 0 ,0 ,0 ,0],
    "2_top" => [4.375,-0.125,0,0, 0 ,0 ,0 ,0],
    "1_top" => [4.375,-0.125,0,0, 0 ,0 ,0 ,0]
  }


  p Compression.max_error(Compression.inverse_haar_transform(test_data["8_top"]), test_data["original"])
  p Compression.max_error(Compression.inverse_haar_transform(test_data["4_top"]), test_data["original"])
  p Compression.max_error(Compression.inverse_haar_transform(test_data["2_top"]), test_data["original"])
  p Compression.max_error(Compression.inverse_haar_transform(test_data["1_top"]), test_data["original"])
end
